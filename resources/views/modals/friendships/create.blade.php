...
<script>
    ...
    const friendshipForm = document.getElementById('new-friendship-form');
    if (friendshipForm) {
        friendshipForm.addEventListener('submit', function(e) {
            ...
            if (!buddyId || !peerBuddyId) {
                alert('Por favor selecciona ambos participantes');
                submitButton.disabled = false;
                submitButton.textContent = 'Crear Emparejamiento';
                return;
            }
            ...
            .then(data => {
                if (data.success) {
                    window.location.reload(); // O actualiza solo la sección de emparejamientos
                } else {
